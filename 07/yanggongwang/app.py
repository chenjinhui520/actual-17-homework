...
import datetime
from userdb import user_delete,change_user_password,get_accesslogs,files_upload
from audit_logs import audit_logs,show_audit_logs

app = Flask(__name__)
app.secret_key = '13ggh@123dfasdf'
...
@app.route('/logs/upload_files/',methods=['POST','GET'])
@login_required
def upload_files():
    my_username = str(session['user']['username'])
    if request.method == 'POST':
        files = request.files.get('filename')
        if files_upload(files):
            audit_logs(my_filename=files.filename, my_path="/tmp", username=my_username)
            flash(u'文件上传成功！')
            return render_template('files.html', rt_list=show_audit_logs())
        else:
            flash(u'文件上传失败！')
    return render_template('files.html', rt_list=show_audit_logs())
