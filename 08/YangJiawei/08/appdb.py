...
@app.route('/user/update_passwd/', methods = ['GET','POST'])
@session_vilidate
def update_passwd():
	name = request.form.get('username')
	passwd = request.form.get('old_passwd')
	new_passwd = request.form.get('new_passwd')
	_isok, error = user.vilidate_update_passwd(name, passwd, new_passwd)
	return json.dumps({'_isok':_isok,'error':error})



@app.route('/user/update_userinfo/', methods = ['GET','POST'])
@session_vilidate
def update_userinfo():
	name = request.form.get('name')
	age = request.form.get('age')
	job = request.form.get('job')
	_isok, error = user.vilidate_update_info(name,age,job)
	return json.dumps({'_isok':_isok,'error':error})


@app.route('/user/add_userinfo/', methods = ['GET','POST'])
@session_vilidate
def add_userinfo():
	name = request.form.get('newuser')
	age = request.form.get('newage')
	job = request.form.get('newjob')
	passwd = request.form.get('passwd')
	_isok, error = user.signup_vilidate(name,passwd,age,job)
	return json.dumps({'_isok':_isok,'error':error})
