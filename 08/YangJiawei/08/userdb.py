...
def vilidate_update_passwd(name, passwd, new_passwd):
	user = get_user(name)
	if user[2] == passwd:
		if len(new_passwd)==0:
			return False, u'用户密码不合法'
		if db.modify_passwd(name,new_passwd):
			return True, u'用户%s密码更新成功'%name
		else:
			return False, u'用户密码更新失败'
	else:
		return False, u'用户密码错误'


def vilidate_update_info(name, age, job):
	if not age :
		return False, u'年龄不能为空'
	elif not job :
		return False, u'职位不能为空'
	else:
		if db.modify_userinfo(name,job,age):
			return True, u'用户%s更新成功'%name
		else:
			return False, u'用户更新失败'
